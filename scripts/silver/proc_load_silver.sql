...
BEGIN
	DECLARE @start_time DATETIME, @end_time DATETIME, @job_start DATETIME, @job_end DATETIME;
	BEGIN TRY
		PRINT '===============================================';
		...
		CAST(
			...
			AS DATE
		) AS prd_end_dt -- Calculate end date as one date before the next start date
		...
		END CATCH
		SET @job_end = GETDATE();
		PRINT '>> Silver Load Duration: ' + CAST (DATEDIFF(second, @job_start, @job_end) AS NVARCHAR) + ' seconds';
END
